...
export default class Cl_principal {
    ...
    iniciarData() {
        ...
        const n0 = libroNino[0];
        if (n0)
            this.controlador.procesarLibroNino(n0);
        const n1 = libroNino[1];
        if (n1)
            this.controlador.procesarLibroNino(n1);
        const n2 = libroNino[2];
        if (n2)
            this.controlador.procesarLibroNino(n2);
        const n3 = libroNino[3];
        if (n3)
            this.controlador.procesarLibroNino(n3);
        const a0 = libroAdolescente[0];
        if (a0)
            this.controlador.procesarLibroAdolescente(a0);
        const a1 = libroAdolescente[1];
        if (a1)
            this.controlador.procesarLibroAdolescente(a1);
        const a2 = libroAdolescente[2];
        if (a2)
            this.controlador.procesarLibroAdolescente(a2);
        const a3 = libroAdolescente[3];
        if (a3)
            this.controlador.procesarLibroAdolescente(a3);
    }
}
